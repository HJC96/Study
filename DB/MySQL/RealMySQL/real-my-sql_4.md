...
**요약 핵심:**
대용량 서비스에서는 `OFFSET`을 사용하는 페이징을 피해야 함. 대신 \*\*마지막 조회 위치(Cursor)\*\*를 기억했다가 그 다음부터 읽는 **No-Offset 방식**을 사용하면, 페이지 깊이와 상관없이 빠르고 일정한 성능을 보장할 수 있음. 정렬 기준 컬럼에 중복이 있을 때는 **PK를 보조 정렬 기준**으로 삼아 데이터 누락을 방지해야 함.
